...
> | P1-09 | 🟡 winrs-cert code+test XONG. **E2E máy thật 2026-07-07**: certstore nạp cert thật + exec phân loại lỗi trong timeout (fix bug treo vô hạn qua goroutine/select). Còn: cert-auth exec THẬT cần WinRM target + admin |
> | **E2E hạ tầng thật** | ✅ **Linux CentOS 7** (server thật): core + plugin manager + go-plugin + /api/plugins PASS (binary static cross-compile, smoke non-invasive). **Windows**: certstore + winrs-cert resilient + bug timeout fix. Doc L5 e2e-real-hardware.md. Còn: full install.sh (systemd+MariaDB) cần VM throwaway |
...
> | **UI (P2-06/P3 UI)** | ✅ **Patch 0006 — trang /openitms** (Vue): Plugins (GET /api/plugins list, e2e PASS) + Registry (search/install) + Hardening (scan/fix). **ui-build CI XANH** (compile + eslint airbnb pass). Còn: Settings config UI (form đổi config) |
